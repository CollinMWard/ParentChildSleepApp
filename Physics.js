...
let tick = 0;
let fenceId = 0;
let nextFenceTick = 1200; // Initialize with a default tick value for fence generation
...
const Physics = (entities, { touches, time, dispatch }) => {
  ...
  // Slowdown logic (after life loss)
  if (entities.physics.slowdownTick) {
    entities.physics.slowdownTick -= delta;

    // If slowdown is active, reduce speed temporarily
    if (entities.physics.slowdownTick > 0) {
      entities.physics.speed = maxSpeedReached * 0.5; // Slow to 50% of the max speed
    } else {
      // Once the slowdown period is over, return to normal speed
      entities.physics.slowdownTick = null;
      entities.physics.speed = maxSpeedReached; // Return to the previous high speed
    }
  }
    ...
    .forEach((key) => {
      ...
      if (distance < 50 && !entity.scored) {
        ...
        if (key.startsWith('pillow')) {
          ...
          entities.physics.slowdownTick = 3000; // Slow down for 3 seconds
  ...
  // Update tick and handle random fence generation time
  tick += delta;
  ...
  if (tick >= nextFenceTick) {
    ...
    // Randomize the time until the next fence appears (between 900ms and 1500ms)
    nextFenceTick = Math.floor(Math.random() * (1300 - 900)) + 1100;
    ...
    // Add shears (1/5 chance)
    if (random >= 0.05 && random < 0.25) {
      const shearYPosition = screenHeight * 0.6 - Math.random() * (screenHeight * 0.2); // Appear between 60%-80% of the screen height
